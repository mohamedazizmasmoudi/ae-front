                ...
                (channelConversations)
                  ? <ul>
                      {channelConversations.map((message, index) => {
                        return (
                          <li className={(username !== message.author[0].item.username || message.author[0].item.userName) ? (username === message.author[0].item.userName) ? "" : "chat--received" : null} key={`chatMsgId-${index}`}>
                            <div className="speech--bubble--author">
                              {
                                (username === message.author[0].item.username )
                                  ? <p>You</p>
                                  : <p>{message.author[0].item.username || message.author[0].item.userName}</p>
                              }
                ...
                (socketConversations)
                  ? <ul>
                      {socketConversations.map((message, index) => {
                        return (
                          <li className={(!message.author) ? "user--joined" : (username !== message.author) ? (username) ? "" : "chat--received" : null}key={`socketMsgId-${index}`}>
                              {
